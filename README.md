# Bazzite HTPC Documentation

Documentation for any and all tools to optimize a Bazzite HTPC to have the most console-like experience! Included are example files for CEC compatability

⠀
## Main
* [Hardware](README.md#Hardware)
* [HDMI CEC compatability](docs/CEC_Compat.md) - wip
* [HDMI 2.1 workaround](docs/hdmi2.1compat.md) -todo>
* USB wake
* Universal Split Screen
* Minor optimizations

⠀
## Hardware used

<sub>Facebook marketplace is not overated! I saved like 60-70% on low-risk components.</sub>  

<ins>***Televisions:***</ins>  
*Samsung S90C, Samsung TU7000D, Samsung Q70C, Sony X800E, LG B6*  
I currently use a Samsung S90C extensively without issue (plenty of user error). I have been able to interact with the above televisions and the process to reproduce was all the same on a basic level.


<ins>***Chassis:***</ins>  
*Silverstone GD05-B*  
Best thing I could find that I wanted due to size constraints in a TV cabinet while wanting 4K120 gaming. Has space for a sata DVD drive as well.  


<ins>***Motherboard:***</ins>  
*MSI MAG B550 TOMAHAWK MAX WIFI*  
The only factor was finding a motherboard with USB-Wake. Unfortunately most motherboard manufacturers don't include that information, even in the manual, so it's either a gamble, or find something that has already been discussed online before.  


<ins>***GPU:***</ins>  
*AMD 7900XT.*  
Each GPU has it's own quirks but it seems AMD will consistently be the most stable.  


<ins>***Others:***</ins>  
These are generally going to be preference based.  
*  Ryzen 7 5800x and TeamForce 32gb RAM. (stability/ram prices)  
* 1TB WD Black m.2 SSD  
* SeaGate internal 4TB HDD.  
* Generic 5.25" optical drive.  
